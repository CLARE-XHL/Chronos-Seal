...
⚠️注意！现有版本有一致命漏洞（system.json在被加密前是明文裸奔），暂不建议添加进生产环境使用，仅适合在本地测试环境运行，该漏洞会在V1.2版本进行封堵。
...
看门狗守护线程：独立系统级线程，10 秒超时，连续 2 次无心跳即强制 exit(0)，封堵 CE 挂起调试

资源加密锁：AES-256-CBC 加密 system.json，随机 IV，密钥永不出现在 JS 内存（仅 C++ 内存）

双存储防篡改（V1.1 新增）：时间哈希同时存入注册表和 auth.key 的 NTFS 备用数据流（ADS），任一被删可自动修复，任一被篡改即触发 ERR_TIME_TAMPER
...
反时间回拨：每次启动检测系统时间是否早于上次成功启动记录的时间。上次启动时间同时写入两处独立位置：auth.key（带 HMAC 签名）和注册表 + NTFS ADS（V1.1 升级为双存储）
...
- 校验该标签是否存在于硬编码的白名单哈希表中
- 单向 HMAC 哈希链迭代：每轮验证通过后，将（上下文 + 盐值）混入 HMAC 迭代，生成新的链值。破解者若内存清零该值，哈希链断裂，后续校验全部失败
- 验证失败时触发强制副作用（修改 JS 层游戏变量如金钱、位置、开关），而非仅返回 false——即使破解者忽略返回值，游戏逻辑本身已被破坏
...
- 首次超时 → 仅标记警告，不动作
- 连续两次超时（即 > 20 秒无心跳）→ 强制调用 exit(0) 闪退
...
HMAC_SALT：openssl rand -base64 32 生成 256 位随机盐

AES_KEY：openssl rand -hex 32 生成 32 字节 AES 密钥

HARD_EXPIRE：date -d "2027-01-01" +%s 设置硬截止时间戳（建议发版日 + 2~3 个月）
...
10 — ERR_EXPIRED
超过硬编码截止日期 → 玩家应前往 Steam 更新游戏。

20 — ERR_ID_MISMATCH
Steam UID 比对失败或采样不一致 → 玩家需确保登录购买账号。

30 — ERR_SIGNATURE
HMAC 签名无效或文件被篡改 → 玩家应重装游戏，或删除 auth.key 重新激活。

31 — ERR_TIME_TAMPER
时间回拨检测触发（注册表 / ADS 校验失败）→ 玩家需同步系统时间后重试。

40 — ERR_NO_RES
system.json.enc 缺失 → 玩家需重装游戏。

50 — ERR_NO_STEAM
Steam 客户端未运行或 SDK 初始化失败（含 5 次重试）→ 玩家需启动 Steam 后重试。
...
首次启动
生成 auth.key 和 system.json.enc，原 system.json 被安全覆写为 0 并删除。

删除注册表
启动后自动补回注册表（V1.1 自修复）。

删除 ADS
执行 echo > auth\auth.key:timestamp，启动后自动重建 ADS（V1.1 自修复）。

同时篡改注册表与 ADS 时间哈希
报 ERR_TIME_TAMPER（31）闪退。

暂停游戏主线程 20 秒以上
看门狗强制退出进程。

换电脑拷贝游戏
Steam UID 不同 → ERR_ID_MISMATCH（20）闪退。

强制改系统时间绕过
硬截止 + 双存储防回拨 → ERR_TIME_TAMPER（31）。

CE 冻结检查点返回值
盐值验算失败 → 触发惩罚（虚空传送 / 清背包）。
...
☐ 对编译后的 .node 模块使用 VMProtect / Themida 进行虚拟化加壳（发布前需充分测试稳定性）

阶段五：打包与发布

☐ 编译 decryptor.node（Release 模式）
☐ 将 .node 打包进加密资源包或使用 Enigma Virtual Box 封装进主程序
...
NW.js 打包若 .node 直接暴露在文件目录，逆向者可单独提取二进制进行静态分析。推荐方案：将 .node 打包进 NW.js 的加密资源包（如使用 nwjc 编译），或使用 Enigma Virtual Box 等打包工具将 .node 封装进主程序壳内，使二进制模块不以独立文件形式暴露于文件系统。

适用场景定位

本方案专为低定价（¥12~¥60）、为爱发电的独立游戏设计。核心价值在于劝退小白、提高门槛，确保文件夹复制传播与简易打包补丁在短时间内自动失效（时间炸弹），保护开发者初期的销售窗口期。
...
· 引入 NTFS 备用数据流（ADS）作为第三隐性存储介质，时间哈希同步写入 auth.key:timestamp
· verify_time 升级为严格双源交叉校验模式（注册表 + ADS），任一存储非空且错误即触发 ERR_TIME_TAMPER（31），空值放过以兼容意外清理
· 调整 GenerateFirstTimeAuth 执行顺序，先写 auth.key 宿主文件再写入 ADS，确保首次激活时 ADS 流正确挂载
...
· 完成整体架构设计，三层防御体系（L1 C++ 信任根 + L2 JS 胶水层 + L3 行为验证层）
...
· ending_trigger → ending_credits 命名统一
· 检查点失败时 C++ 层主动注入游戏状态惩罚（清金钱、传送原点、破坏开关），不再仅返回布尔值
· 移除 verify_time 中 last_seen_time > 0 的多余判断
· 哈希链由 XOR 迭代升级为单向 HMAC-SHA256 迭代
· Steam 初始化重试提升至 5 次、间隔 100ms；g_last_salt 提升为全局原子变量
· 新增 .overwrite_failed 覆写失败持久化标记与启动重试逻辑
· 新增看门狗日志编译宏 WATCHDOG_LOGGING，Release 模式默认关闭
